...
const dateLabel = (value: unknown) =>
  toDateObj(value)?.toLocaleDateString(undefined, {
    month: "short",
    day: "numeric",
    year: "numeric",
  }) ?? "Date not set";
...
export default function SettingsInjuriesScreen() {
  ...
  const [savedDraft, setSavedDraft] = useState<FormDraft>(() =>
    draftFromInjury(),
  );
    ...
    try {
      ...
      setInjuries(
        stored
          ...
          .filter((injury) => isBodyPart(injury?.bodyPart)),
      );
    } catch (error) {
      console.error(error);
      setLoadError(true);
  ...
  const selected =
    view.kind === "detail" || view.kind === "form"
      ? injuries.find((injury) => injury.id === view.id)
      : undefined;
  const ongoing = useMemo(
    () => injuries.filter((injury) => injury.status === "ongoing"),
    [injuries],
  );
  const past = useMemo(
  ...
  const goBackFromForm = () => {
    ...
    setView(
      view.kind === "form" && view.origin === "detail" && view.id
  ...
  const saveDraft = async () => {
    ...
    if (
      ...
      dateValue(draft.resolvedDate) < dateValue(draft.onsetDate)
    ) {
    ...
    const injury: Injury = {
      ...
      ...(draft.status === "resolved"
        ? { resolvedDate: draft.resolvedDate.toISOString() }
        : { resolvedDate: null }),
    ...
    try {
      ...
      setInjuries((current) =>
        existing
          ? current.map((item) => (item.id === injury.id ? injury : item))
          : [...current, injury],
      );
  ...
  const markEnded = async (injury: Injury) => {
    ...
    try {
      ...
      triggerSyncAfterWrite();
      setInjuries((current) =>
        current.map((item) => (item.id === injury.id ? updated : item)),
      );
  ...
  const applyHistory = async (injury: Injury) => {
    ...
    try {
      ...
      triggerSyncAfterWrite();
      showToast(
        `Added ${count} workout${count === 1 ? "" : "s"} from ${dateLabel(injury.onsetDate)} to ${injury.resolvedDate ? dateLabel(injury.resolvedDate) : "today"}`,
      );
    } catch (error) {
      console.error(error);
  ...
  const deleteInjury = async () => {
    ...
    try {
      ...
      triggerSyncAfterWrite();
      setInjuries((current) =>
        current.filter((injury) => injury.id !== selected.id),
      );
  ...
  function renderRow(injury: Injury) {
    return (
      <TouchableOpacity
        key={injury.id}
        accessibilityRole="button"
        accessibilityLabel={`${bodyPartLabel(injury.bodyPart)}, ${sideLabel(injury.side)}, ${cap(injury.severity)}`}
        style={styles.row}
        onPress={() => setView({ kind: "detail", id: injury.id })}
      >
        <View style={styles.rowCopy}>
          <Text style={styles.rowTitle}>
            {bodyPartLabel(injury.bodyPart)} · {sideLabel(injury.side)}
          </Text>
          <Text style={styles.rowMeta}>
            {cap(injury.severity)} · {dateLabel(injury.onsetDate)}
            {injury.resolvedDate
              ? ` – ${dateLabel(injury.resolvedDate)}`
              : " – ongoing"}
          </Text>
        </View>
  ...
  const renderList = () => (
    <>
      ...
      <FadingScrollView contentContainerStyle={styles.content}>
        ...
        >
          <Text style={styles.primaryButtonText}>Add injury</Text>
        </TouchableOpacity>
        {loading ? (
          <ActivityIndicator
          ...
          <View style={styles.messageBlock}>
            <Text style={styles.empty}>Could not load injuries.</Text>
            <TouchableOpacity
          ...
          <>
            ...
            {ongoing.length ? (
              ...
              <Text style={styles.empty}>No current injuries.</Text>
            )}
            <TouchableOpacity
              accessibilityRole="button"
    ...
    <>
      ...
      <FadingScrollView contentContainerStyle={styles.content}>
        ...
        <Text style={styles.detailStatus}>
          {injury.status === "ongoing" ? "Current injury" : "Past injury"} ·{" "}
          {cap(injury.severity)}
        </Text>
        <View style={styles.detailCard}>
          <Text style={styles.detailLabel}>Date started</Text>
          <Text style={styles.detailValue}>{dateLabel(injury.onsetDate)}</Text>
          {injury.resolvedDate ? (
            <>
              <Text style={styles.detailLabel}>Date ended</Text>
              <Text style={styles.detailValue}>
                {dateLabel(injury.resolvedDate)}
              </Text>
            </>
          ) : null}
          <Text style={styles.detailLabel}>Past workout range</Text>
          <Text style={styles.detailValue}>
            {dateLabel(injury.onsetDate)} –{" "}
            {injury.resolvedDate ? dateLabel(injury.resolvedDate) : "today"}
          </Text>
          <Text style={styles.detailHint}>
            Adding to past workouts only affects completed workouts in this
            saved date range.
          </Text>
          {injury.avoid?.length ? (
            <>
              <Text style={styles.detailLabel}>Movements to avoid</Text>
              <Text style={styles.detailValue}>{injury.avoid.join(", ")}</Text>
            </>
          ) : null}
          {injury.notes ? (
            <>
              <Text style={styles.detailLabel}>Notes</Text>
              <Text style={styles.detailValue}>{injury.notes}</Text>
            </>
          ) : null}
        </View>
        <TouchableOpacity
          style={styles.primaryButton}
          onPress={() => openEdit(injury)}
          disabled={busy}
        >
          <Text style={styles.primaryButtonText}>Edit injury</Text>
        </TouchableOpacity>
        {injury.status === "ongoing" ? (
          <TouchableOpacity
            style={styles.secondaryButton}
            onPress={() => void markEnded(injury)}
            disabled={busy}
          >
            <Text style={styles.actionText}>Mark as ended</Text>
          </TouchableOpacity>
        ) : null}
        <TouchableOpacity
          style={styles.secondaryButton}
          onPress={() => void applyHistory(injury)}
          disabled={busy}
        >
          <Text style={styles.actionText}>Add to past workouts</Text>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          disabled={busy}
        >
          <Text style={styles.dangerText}>Delete injury</Text>
        </TouchableOpacity>
      </FadingScrollView>
    </>
  );
  ...
  const renderForm = () => (
    <>
      {header(
        view.kind === "form" && view.id ? "Edit injury" : "Add injury",
        goBackFromForm,
      )}
      <FadingScrollView contentContainerStyle={styles.content}>
        <Text style={styles.formIntro}>
          {view.kind === "form" && view.id
            ? "Update the details below. Saving does not change past workouts."
            : "Save the injury first, then decide whether to add it to past workouts."}
        </Text>
        <Text style={styles.fieldLabel}>Body part</Text>
        <Dropdown
          accessibilityLabel="Body part"
          options={BODY_PARTS.map(bodyPartLabel)}
          value={draft.bodyPart ? bodyPartLabel(draft.bodyPart) : null}
          onSelect={(value) =>
            setDraft((current) => ({
              ...current,
              bodyPart:
                BODY_PARTS.find((part) => bodyPartLabel(part) === value) ??
                null,
            }))
          }
          placeholder="Choose a body part"
        ...
        <Dropdown
          ...
          value={sideLabel(draft.side)}
          onSelect={(value) =>
            setDraft((current) => ({ ...current, side: sideFromLabel(value) }))
          }
          placeholder="Side"
          style={styles.fieldControl}
        />
        ...
        <Dropdown
          ...
          value={cap(draft.severity)}
          onSelect={(value) =>
            setDraft((current) => ({
              ...current,
              severity: value.toLowerCase() as InjurySeverity,
            }))
          }
          placeholder="Severity"
          style={styles.fieldControl}
        />
        <Text style={styles.fieldLabel}>Date started</Text>
        <DateField
          value={draft.onsetDate}
          onChange={(date) =>
            setDraft((current) => ({ ...current, onsetDate: atNoon(date) }))
          }
        />
        <Text style={styles.fieldLabel}>Status</Text>
        <View style={styles.statusOptions}>
          {(["ongoing", "resolved"] as const).map((status) => (
            <TouchableOpacity
              key={status}
              ...
              style={[
                styles.statusOption,
                draft.status === status && styles.statusOptionSelected,
              ]}
        ...
        {draft.status === "resolved" ? (
          <>
            <Text style={styles.fieldLabel}>Date ended</Text>
            <DateField
              value={draft.resolvedDate}
              onChange={(date) =>
                setDraft((current) => ({
                  ...current,
                  resolvedDate: atNoon(date),
                }))
              }
            />
          </>
        ) : null}
        <Text style={styles.fieldLabel}>Movements to avoid (optional)</Text>
        <TextInput
          accessibilityLabel="Movements to avoid"
          style={styles.input}
          placeholder="e.g. overhead press, dips"
          placeholderTextColor="#777"
          value={draft.avoidText}
          onChangeText={(avoidText) =>
            setDraft((current) => ({ ...current, avoidText }))
          }
        />
        ...
        >
          {busy ? (
            ...
            <Text style={styles.primaryButtonText}>Save injury</Text>
          )}
        </TouchableOpacity>
      </FadingScrollView>
    </>
      ...
      >
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>Delete injury?</Text>
            <Text style={styles.modalBody}>
              This deletes the injury and removes its link from every workout.
            ...
            <View style={styles.modalRow}>
              <TouchableOpacity
                ...
                disabled={busy}
              >
                <Text style={styles.actionText}>Keep injury</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.dangerButton}
                onPress={() => void deleteInjury()}
                disabled={busy}
              >
                <Text style={styles.dangerText}>
                  {busy ? "Deleting…" : "Delete injury"}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
      ...
      >
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalBody}>
              Your unsaved injury changes will be lost.
            </Text>
            <View style={styles.modalRow}>
              <TouchableOpacity
                style={styles.secondaryButton}
              ...
              >
                <Text style={styles.actionText}>Keep editing</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.dangerButton}
                onPress={discardForm}
              >
                <Text style={styles.dangerText}>Discard changes</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
