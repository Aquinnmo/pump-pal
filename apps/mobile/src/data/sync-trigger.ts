...
// Thin binding layer (like src/data/client.ts, src/data/sync.ts) — not unit-testable via
// tsx since it imports react-native/NetInfo/TaskManager; the mutex coalescing
// it relies on IS tested, in src/data/keyed-mutex.test.ts.
//
// Web build: src/data/sync-trigger.web.ts — same exported function names, all
 ...
 * Genuinely concurrent runs are already coalesced by src/data/sync.ts's per-uid mutex.
 *
