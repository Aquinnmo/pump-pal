...
// just a convention — see src/models/remote-types.ts.
//
// Rules for adding a migration:
// - Never edit a migration that has shipped. Append a new { version, up }
//   entry with version = previous max + 1.
// - Each `up` statement runs inside one transaction (src/models/migrate.ts) that also
...
//   version (see src/models/migrate.test.ts for the rollback case).
...
export const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    up: [
      ...
      // Singletons key their outbox intent by uid — see src/models/singleton-repository.ts.
      `INSERT OR IGNORE INTO outbox (id, uid, entity_type, entity_id, op, payload, base_version, created_at, attempts)
