...
// src/models/schema.ts), so one factory implements src/models/remote-types.ts's
// LocalSingletonRepository<T> for either instead of two near-duplicate files.
import { SqlExecutor } from './executor';
import { LocalSingletonRepository, StoredRecord, SyncMeta } from '@/models/remote-types';
