...
// Thin binding layer (like src/models/client.ts, src/models/sync.ts) — not unit-testable via
// tsx since it imports react-native/NetInfo/TaskManager; the mutex coalescing
// it relies on IS tested, in src/models/keyed-mutex.test.ts.
//
// Web build: src/models/sync-trigger.web.ts — same exported function names, all
// no-ops, so web never bundles NetInfo/TaskManager/BackgroundTask and
// src/context/auth-context.tsx can call this uniformly on both platforms.
...
/** Called once from src/context/auth-context.tsx so this module never imports Firebase itself. */
 ...
 * Genuinely concurrent runs are already coalesced by src/models/sync.ts's per-uid mutex.
 *
