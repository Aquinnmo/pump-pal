...
// Web build of src/models/sync-trigger.ts. Web is directly API-backed per request
// (repositories/*.web.ts) — no outbox, no background sync, nothing to
// trigger. Same exported names as the native file, all no-ops, so
// src/context/auth-context.tsx can call this uniformly without a Platform.OS
