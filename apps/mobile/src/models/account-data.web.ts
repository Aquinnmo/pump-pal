// Web build of src/models/account-data.ts. Web repositories read and write through
// to Firestore per request, so there is no outbox to drain — but there is now a
// session read cache (src/models/web-read-cache.ts) that must not survive a sign-out.
