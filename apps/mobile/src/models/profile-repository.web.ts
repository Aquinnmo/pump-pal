// Web build of src/models/profile-repository.ts. Same LocalSingletonRepository<UserDoc>
// shape as native, but `upsert` only ever forwards `workoutSplit` and
// `aiEnabled`/`socialEnabled` — Firestore rules allowlist these fields on purpose
...
// repository instead.
import { LocalSingletonRepository, StoredRecord } from '@/models/remote-types';
import { createVersionCache } from '@/models/version-cache';
...
function toStoredRecord(dto: ProfileDTO): StoredRecord<UserDoc> {
  ...
  return {
    ...
    data: {
      ...
      socialEnabled: dto.socialEnabled ?? undefined,
    },
    syncState: 'synced',
...
export const profileRepository: LocalSingletonRepository<UserDoc> = {
  ...
  async upsert(_uid: string, entity: UserDoc): Promise<void> {
    ...
    // A split-only upsert never rewrites either preference (and vice versa).
    const patch = {
      ...
      ...(entity.socialEnabled === undefined ? {} : { socialEnabled: entity.socialEnabled }),
    };
