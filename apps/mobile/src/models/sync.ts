// Real wiring for the sync engine (src/models/sync-engine.ts): binds it to the
// native src/models/client.ts, the local workout/profile/pushup repositories, and
// repositories/remote/*.ts. This is what src/models/sync-trigger.ts calls — never
// src/models/sync-engine.ts directly.
//
...
import * as remoteCatalog from '@/models/remote/catalog';
...
// One in-flight run per uid — see src/models/keyed-mutex.ts. A call for a
...
export async function syncNow(
  ...
  if (!currentUid || currentUid !== uid) {
    ...
    // caller (src/models/sync-trigger.ts) passes the live Firebase auth uid so this
