// Web build of src/models/workout-repository.ts (Metro picks this file automatically
...
// The remote list arrives ordered by updatedAt (src/models/firestore-sync-remote.ts) —
// right for sync cursors, wrong for display. Native gets its order from SQL
// (src/models/workouts.ts:53/69 date DESC, :81 date ASC) and callers rely on it:
