// Native Google sign-in. Web build: src/lib/google-sign-in.web.ts — same two
// exports backed by signInWithPopup, so the native module never reaches the
// web bundle (same Metro platform-extension split as src/config/firebase.web.ts).
//
...
import { assertGoogleLinkIdentity } from '@/lib/google-account-link';
