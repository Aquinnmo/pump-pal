...
assert.deepEqual(nextUsage(undefined, TODAY, LIMIT), { date: TODAY, count: 1 });

// Within the same day the counter walks up to the limit.
assert.deepEqual(nextUsage({ date: TODAY, count: 1 }, TODAY, LIMIT), { date: TODAY, count: 2 });
assert.deepEqual(nextUsage({ date: TODAY, count: 2 }, TODAY, LIMIT), { date: TODAY, count: 3 });

// At the limit, the claim is refused.
assert.equal(nextUsage({ date: TODAY, count: LIMIT }, TODAY, LIMIT), null);

// Defensive: a counter above the limit (manual edit, changed limit) stays refused.
assert.equal(nextUsage({ date: TODAY, count: 99 }, TODAY, LIMIT), null);

// A stale record rolls over instead of blocking — this is what makes the quota
// reset daily without a scheduled job.
assert.deepEqual(nextUsage({ date: YESTERDAY, count: LIMIT }, TODAY, LIMIT), { date: TODAY, count: 1 });
