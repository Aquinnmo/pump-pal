...
/**
 * Path -> handler table for the single serverless function (`api/index.ts`).
 *
 * Vercel counts every file under this package's `api/` directory as its own
 * serverless function, and the Hobby plan caps a deployment at 12. The handlers
 * therefore live under `src/routes/`, outside that scan, and this table
 * reproduces what filename-based routing used to do.
 *
