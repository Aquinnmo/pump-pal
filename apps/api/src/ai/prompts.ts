...
import { AI_MAX_RETRIES, AI_OPS, type AIOp, type AIOpInput, type AIOpOutput } from '@timber/contract/ai';
import { getAIModel } from './model.js';

/**
 * Server-owned prompt templates, one per operation. Each reads its output
 * schema from `AI_OPS[op].output` (packages/contract/src/ai-contract.ts) instead of a
