...
import * as remoteInjuries from '@/repositories/remote/injuries';
import * as remoteProfile from '@/repositories/remote/profile';
...
import { CreateWorkoutInput, UpdateWorkoutInput, WorkoutDTO, PullRequest, InjuryDTO, ProfileDTO, PushupChallengeDTO } from '@/shared/api-contract';
...
// Mirrors db/workout-repository.web.ts's toCreateInput — kept separate
...
// One in-flight run per uid — see db/keyed-mutex.ts. A call for a
...
export async function syncNow(
  ...
  if (!currentUid || currentUid !== uid) {
    ...
    // caller (db/sync-trigger.ts) passes the live Firebase auth uid so this
